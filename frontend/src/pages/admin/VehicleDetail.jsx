...
// ─── Add Trip modal (vehicle pre-filled) ──────────────────────────────────────
        ...
        <div className="flex items-center justify-between border-b border-gray-100 p-5">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              ...
              {primaryDriver && <> · Driver: <strong>{primaryDriver.name}</strong></>}
            </p>
          </div>
          <button onClick={onClose} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100">✕</button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input type="date" required value={form.trip_date} max={new Date().toISOString().split('T')[0]}
                onChange={(e) => set('trip_date', e.target.value)} className={fieldClass} />
            </div>
            <div>
          ...
          <div className="rounded-xl border border-gray-200 p-4 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <input type="number" required min="0.1" step="0.1" value={form.one_way_km_1}
                  onChange={(e) => set('one_way_km_1', e.target.value)} className={fieldClass} placeholder="e.g. 26" />
              </div>
              <div>
          ...
          {showStore2 ? (
            <div className="rounded-xl border border-emerald-200 p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => { setShowStore2(false); set('store_name_2', ''); set('one_way_km_2', ''); }}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-600">One-way KM</label>
                  <input type="number" min="0.1" step="0.1" value={form.one_way_km_2}
                    onChange={(e) => set('one_way_km_2', e.target.value)} className={fieldClass} />
                </div>
                <div>
          ...
          <div>
            ...
            <textarea rows={2} value={form.remarks} onChange={(e) => set('remarks', e.target.value)}
              className={`${fieldClass} resize-none`} placeholder="Any notes…" />
          </div>
...
// ─── Add Expense modal (vehicle pre-filled) ───────────────────────────────────
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 overflow-y-auto">
      <div className="my-8 w-full max-w-md rounded-2xl bg-white shadow-xl">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave({ vehicle_id: vehicle.id, driver_id: primaryDriver?.id, ...form }); }}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              <input type="number" required min="0.01" step="0.01" value={form.amount}
                onChange={(e) => set('amount', e.target.value)} className={fieldClass} placeholder="0.00" />
            </label>
            <label className="block">
              ...
              <input type="date" required value={form.expense_date} max={new Date().toISOString().split('T')[0]}
                onChange={(e) => set('expense_date', e.target.value)} className={fieldClass} />
            </label>
          </div>
          <label className="block">
          ...
          <label className="block">
            ...
            <textarea rows={2} value={form.description} onChange={(e) => set('description', e.target.value)}
              className={`${fieldClass} resize-none`} placeholder="Optional notes…" />
          </label>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="rounded-xl bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200">Cancel</button>
...
const VehicleDetail = () => {
  ...
  const handleAddTrip = (payload) => {
    createTrip(payload, {
      onSuccess: () => { setShowAddTrip(false); refetchTrips(); },
    });
  };

  const handleAddExpense = (payload) => {
    createExpense(payload, {
      onSuccess: () => { setShowAddExpense(false); refetchExpenses(); },
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {showAddExpense && (
        <AddExpenseModal vehicle={vehicle} onClose={() => setShowAddExpense(false)} onSave={handleAddExpense} isSaving={isCreatingExpense} />
      )}
      {deleteExpenseId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button onClick={() => deleteExpense(deleteExpenseId, { onSuccess: () => { setDeleteExpenseId(null); refetchExpenses(); } })}
                disabled={isDeleting} className="flex-1 rounded-xl bg-red-600 py-2 text-sm font-semibold text-white disabled:bg-gray-300">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-4">
          <button onClick={() => navigate('/admin/vehicles')}
            className="mt-1 rounded-xl border border-gray-200 p-2 text-gray-500 hover:bg-gray-100">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          {primaryDriver ? (
            <>
              <div className="text-base font-bold text-gray-900">{primaryDriver.name}</div>
              <div className="text-sm text-gray-500 mt-0.5">{primaryDriver.phone || 'No phone'}</div>
            </>
          ) : (
            <div className="text-sm text-gray-400 italic">No driver assigned</div>
          )}
          {allDrivers.length > 1 && (
            <div className="mt-2 text-xs text-gray-400">{allDrivers.length - 1} other driver(s) assigned</div>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-gray-500 mb-3">
            <Truck className="h-3.5 w-3.5" /> Total KM
          </div>
      ...
      <div className="flex gap-1 border-b border-gray-200">
        {[
          { key: 'trips', label: `Trips (${trips.length})`, icon: MapPin },
      ...
      {activeTab === 'expenses' && (
        <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-5">
              {Object.entries(
                expenses.reduce((acc, e) => {
                  acc[e.expense_type] = (acc[e.expense_type] || 0) + parseFloat(e.amount || 0);
                  return acc;
                }, {})
              ).map(([type, amt]) => (
          ...
          <div className="flex flex-wrap items-center gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            {expenseFilter.expense_type && (
              <button onClick={() => setExpenseFilter({ expense_type: '' })} className="text-sm text-blue-600 hover:underline">Clear</button>
            )}
          </div>
              ...
              {expenses.map((expense) => (
                <div key={expense.id} className="flex items-center justify-between rounded-xl border border-gray-100 bg-white px-5 py-4 shadow-sm">
                  ...
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    {expense.is_deducted && <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700">Deducted</span>}
                    <button onClick={() => setDeleteExpenseId(expense.id)}
